...
class BPPBHelloBlock{
	...
	function onInit() {
		...
		wp_set_script_translations( 'bppb-hello-editor-script', 'table-of-contents', BPPB_DIR_PATH . 'languages' );
	}
	...
	function render( $attributes ){
		...
		wp_set_script_translations( 'bppb-hello-script', 'table-of-contents', BPPB_DIR_PATH . 'languages' );

		$className = $className ?? '';
		$blockClassName = "wp-block-bppb-hello $className align$align";

		ob_start(); ?>
		<div class='<?php echo esc_attr( $blockClassName ); ?>' id='bppbHelloBlock-<?php echo esc_attr( $cId ) ?>' data-attributes='<?php echo esc_attr( wp_json_encode( $attributes ) ); ?>'></div>
